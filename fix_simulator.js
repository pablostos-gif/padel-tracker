const fs = require('fs');
let c = fs.readFileSync('padel-tracker.html', 'utf8');

const startMarker = '// TACTICS BOARD\n// ═══════════════════════════════════════════════════════════════';
const endMarker = '</script>\n</body>\n</html>';

const si = c.indexOf(startMarker);
const ei = c.lastIndexOf(endMarker);

if (si < 0 || ei < 0) { console.log('ERROR markers not found', si, ei); process.exit(1); }

const newJS = `// TACTICS BOARD
// ═══════════════════════════════════════════════════════════════
...
function drawCourt(ctx){
  ...
  ctx.fillRect(PX,PT,IW,10); ctx.fillRect(PX,PT+IH-10,IW,10);
  ctx.fillRect(PX,PT+10,10,IH-20); ctx.fillRect(PX+IW-10,PT+10,10,IH-20);
...
function drawPlayer(ctx,id,fill){
  const [ex,ey]=pos[id]; const x=px(ex), y=py(ey);
  ...
  ctx.fillText(name.substring(0,5),x,y); ctx.textBaseline='alphabetic';
...
function drawBall(ctx){
  const [ex,ey]=pos.ball; const x=px(ex), y=py(ey);
  ctx.fillStyle='#f4a226'; ctx.beginPath(); ctx.arc(x,y,BR,0,Math.PI*2); ctx.fill();
...
function drawArrow(ctx,x1,y1,x2,y2,color,dashed){
  ...
  ctx.setLineDash([]);
  const angle=Math.atan2(dy,dx); ctx.fillStyle=color;
  ctx.beginPath();
  ctx.moveTo(x2,y2);
...
function exportImg(){
  ...
  ctx.font='bold 11px Arial'; ctx.fillStyle='rgba(255,255,255,0.45)'; ctx.textAlign='center';
  ...
  if(note){
    ...
    const words=note.split(' '); let line='', ly=CH+34;
    words.forEach(w=>{
      const test=line+w+' ';
      if(ctx.measureText(test).width>CW-18&&line!==''){ctx.fillText(line,9,ly);line=w+' ';ly+=15;}
      else line=test;
    });
    ctx.fillText(line,9,ly);
  }
...
const newContent = c.substring(0, si) + newJS + '\n' + endMarker;
fs.writeFileSync('padel-tracker.html', newContent, 'utf8');
console.log('OK - written', newContent.length, 'chars');
